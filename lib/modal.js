  ...
  render: function() {
    ...
    return (
      <div className='modal pad4 fill-white keyline-all round' id='settings'>
        ...
        <form id='form-settings' onSubmit={self.handleSubmit}>
          <fieldset name='loc'>
            <label>Initial Position</label>
            <div className='radio-pill pill clearfix col12' name='locationSettings'>
              {self.state.locationSettings.map(function(item) {
                return <span>
                <input type='radio' name='locationSettings' value={item.id} id={item.id} checked={item.checked} onChange={self.handleChangeLocation.bind(self, item.id)} />
          ...
          <fieldset  name='map'>
            ...
            <div className='radio-pill pill clearfix col12' name='map'>
              {self.state.mapSettings.map(function(item) {
                return <span><input type='radio' name='map' value={item.id} id={item.id} checked={item.checked} onChange={self.handleChangeMap.bind(self, item.id)} /><label htmlFor={item.id} className='col3 button icon check'>{item.name}</label></span>
              })}
            </div>
          </fieldset>
          <fieldset>
            ...
            <input type='text' defaultValue={this.props.radomMapId} className='col6' name='custom-mapid' ref='customMapId' disabled placeholder="Enable 'Custom' to edit" />
          </fieldset>
