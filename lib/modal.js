...
var Modal = React.createClass({
  ...
  handleChangeTimeDisplay: function(id) {
    var timeDisplaySettings = this.state.timeDisplaySettings.map(function(d) {
      return {
        id: d.id,
        checked: (d.id === id ? true : false),
        name: d.name
      ...
      <div className='modal pad4 fill-white keyline-all round' id='settings'>
        ...
        <form id='form-settings' onSubmit={this.handleSubmit}>
          <fieldset name='time'>
            ...
            <div className='radio-pill pill clearfix col12' name='timeDisplaySettings'>
              {this.state.timeDisplaySettings.map(item => {
                return <span key={item.id}>
                <input type='radio' name='timeDisplaySettings' value={item.id} id={item.id} checked={item.checked} onChange={this.handleChangeTimeDisplay.bind(this, item.id)} />
